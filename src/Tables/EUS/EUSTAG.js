...
export default function EUSTAG() {
  ...
  const handleConcatenate = (e) => {
    ...
    const concatenatedResult1 = `HE${newTagNumber}EUS${OemCodes[oem]}${DEorLA[dela]}${LocationCodes[location]}`;
    setResult1(concatenatedResult1);
    navigate('/addEUSasset', { state: { result1: concatenatedResult1} });
        ...
        <div className="col-md-6 offset-md-3 border rounded p-4 mt-2 shadow">
          ...
          {isForm1Visible ? (
            <form onSubmit={handleConcatenate}>
              ...
              <div className="mb-3 custom-dropdown">
                <label htmlFor="dela" className="form-label">Desktop/Laptop:</label>
                <select
                  id="dela"
                  className="form-control"
                  value={dela}
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <button type="button" className="btn btn-outline-primary" onClick={() => navigate('/addEUSasset')}>
                  Go to Add EUS Asset
                </button>
              </div>
            </div>
