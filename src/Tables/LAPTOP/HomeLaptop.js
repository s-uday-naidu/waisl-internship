...
export default function Homelaptop() {
    ...
    const deleteUser = async (tagNumber) => {
        ...
        if (confirmed) {
            try {
                await axios.delete(`http://localhost:8080/api/v12/laptops/asset/${tagNumber}`);
                setlaptopList(laptopList.filter(laptop => laptop.asset_tag_number !== tagNumber));
            } catch (error) {
                            ...
                            <tbody>
                                {laptopList.map((laptop, index) => (
                                    <tr key={laptop.id || index}>
                                        ...
                                        <td>{laptop.status}</td>
                                        <td>
                                            <Link className="btn btn-outline-primary btn-sm mx-1" to={`/editlaptop/${laptop.asset_tag_number}`}>
                                                <FontAwesomeIcon icon={faEdit} />
                                            </Link>
                                            <button
                                                className="btn btn-outline-danger btn-sm mx-1"
                                                onClick={() => deleteUser(laptop.asset_tag_number)}
                                            >
