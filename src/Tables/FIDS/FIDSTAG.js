...
export default function FIDSTAG() {
  ...
  const handleConcatenate = (e) => {
    ...
    const concatenatedResult1 = `HC${newTagNumber}FID${ScreenCodes[sc]}SC${LocationCodes[location]}`;
    const concatenatedResult2 = `HC${newTagNumber}FID${OemCodes[oem]}TC${LocationCodes[location]}`;
    setResult1(concatenatedResult1);
    setResult2(concatenatedResult2);
    navigate('/AddFidsasset', { state: { result1: concatenatedResult1, result2: concatenatedResult2 } });
          ...
          {isForm1Visible ? (
            <form onSubmit={handleConcatenate}>
              <div className="mb-3 custom-dropdown">
                <label htmlFor="oem" className="form-label">TC-OEM:</label>
                <select
                  id="oem"
                  ...
                  required
                >
                  <option value="">Select Thin Client's OEM</option>
              ...
              <div className="mb-3 custom-dropdown">
                <label htmlFor="sc" className="form-label">SC-OEM:</label>
                <select
                  id="sc"
                  className="form-control"
                  value={sc}
                ...
                >
                  ...
                  {Object.keys(ScreenCodes).map((code) => (
                    <option key={code} value={code}>{code}</option>
                  ))}
                </select>
              </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <button type="button" className="btn btn-outline-primary" onClick={() => navigate('/AddFidsasset')}>
                  Go to Add FIDS Asset
                </button>
              </div>
            </div>
          )}
