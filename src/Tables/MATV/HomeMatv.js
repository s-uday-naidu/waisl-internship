...
export default function HomeMatv() {
    ...
    const deleteUser = async (tagNumber) => {
        ...
        if (confirmed) {
            try {
                await axios.delete(`http://localhost:8080/api/v7/matv/deleteByAssetTag/${tagNumber}`);
                setMatvList(MatvList.filter(Matv => Matv.asset_tag_number !== tagNumber));
            } catch (error) {
                            ...
                            <tbody>
                                {MatvList.map((Matv, index) => (
                                    <tr key={Matv.id || index}>
                                        ...
                                        <td>
                                            <Link className="btn btn-outline-primary btn-sm mx-1" to={`/EditMatv/${Matv.asset_tag_number}`}>
                                                <FontAwesomeIcon icon={faEdit} />
                                            </Link>
                                            <button
                                                className="btn btn-outline-danger btn-sm mx-1"
                                                onClick={() => deleteUser(Matv.asset_tag_number)}
                                            >
